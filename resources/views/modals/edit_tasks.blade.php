...
<body>
    ...
    <section>
                ...
                @foreach ($tasks as $task)
                    <div class="todo_wrapper">
                        <div style="display: flex; align-items: center; padding: 15px 10px;">
                            <form action="{{ route("done-tasks", $task->id) }}" method="POST">
                                @csrf
                                @method("PUT")
                        ...
                        <div style="margin-right: 10px; display: flex; align-items: center;">
                            <a style="background: none; border: none; cursor: pointer;" href="{{ route("exact-tasks", $task -> id) }}"><i class="far fa-edit" style="color: #2a9d8f; font-size: 18px; margin: 0 10px 0 0;"></i></a>
                            <form action="{{ route("delete-tasks", $task->id) }}" method="get">
                                @csrf
            ...
            @if (count($done_tasks) > 0)
                <div>
                    <div style="display: flex; padding: 5px; width: 115px; cursor: pointer; border-radius: 5px; background: #212529; margin: 0 0 10px 0;" class="Done_Myday_Controller">
                        ...
                        <p style="margin: 0 0 0 5px; color: white;">Completed {{ count($done_tasks) }}</p>
                    </div>
                    <div id="Done_Myday" style="display: flex; flex-direction: column;">
                        @foreach ($done_tasks as $done_task)
                            <div class="todo_wrapper">
                                <div style="display: flex; align-items: center; padding: 15px 10px;">
                                    <form action="{{ route("undone-tasks", $done_task->id) }}" method="POST">
                                        @csrf
                                        @method("PUT")
                                    ...
                                    <p style="text-decoration: line-through; color: #ced4da;">{{ $done_task->description }}</p>
                                </div>
                                <div style="margin-right: 10px; display: flex; align-items: center;">
                                    <a style="background: none; border: none; cursor: pointer;" href="{{ route("exact-tasks", $done_task -> id) }}"><i class="far fa-edit" style="color: #2a9d8f; font-size: 18px; margin: 0 10px 0 0;"></i></a>
                                    <form action="{{ route("delete-tasks", $done_task->id) }}" method="get">
                                        @csrf
                                        <button class="show_confirm" type="submit" style="background: none; border: none; cursor: pointer;"><i class="fas fa-trash-alt" style="color: tomato; font-size: 18px;"></i></button>
                                    </form>
                                </div>
        ...
        <div>
            <div class="todo_wrapper">
                <form action="{{ route("create-tasks") }}" method="post" class="todos">
                    @csrf
                    <div class="todo">
    ...
    <div class="edit_modal_wrapper">
        <div class="edit_modal">
            <form action="{{ route("update-tasks", $exact_task->id) }}" method="POST">
                <div class="description_block">
                    <input type="text" value="{{ $exact_task->description }}" name="description">
                </div>
        ...
        <div class="created_date">
            <p>Create on {{ $exact_task -> created_at }}</p>
            <form action="{{ route("delete-tasks", $exact_task->id) }}" method="GET">
