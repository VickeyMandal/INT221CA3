...
<body>
    ...
    <section>
                ...
                @foreach ($importants as $important)
                    <div class="todo_wrapper">
                        ...
                        <div style="margin-right: 10px; display: flex; align-items: center;">
                            <a style="background: none; border: none; cursor: pointer;" href="{{ route("exact-important", $important -> id) }}"><i class="far fa-edit" style="color: #2a9d8f; font-size: 18px; margin: 0 10px 0 0;"></i></a>
                            <form action="{{ route("delete-important", $important->id) }}" method="get">
                                @csrf
            ...
            @if (count($done_importants) > 0)
                <div>
                    <div style="display: flex; padding: 5px; width: 115px; cursor: pointer; border-radius: 5px; background: #212529; margin: 0 0 10px 0;" class="Done_Myday_Controller">
                        ...
                        <p style="margin: 0 0 0 5px; color: white;">Completed {{ count($done_importants) }}</p>
                    </div>
                    <div id="Done_Myday" style="display: flex; flex-direction: column;">
                        @foreach ($done_importants as $done_important)
                            <div class="todo_wrapper">
                                <div style="display: flex; align-items: center; padding: 15px 10px;">
                                    <form action="{{ route("undone-important", $done_important->id) }}" method="POST">
                                        @csrf
                                        @method("PUT")
                                    ...
                                    <p style="text-decoration: line-through; color: #ced4da;">{{ $done_important->description }}</p>
                                </div>
                                <div style="margin-right: 10px; display: flex; align-items: center;">
                                    <a style="background: none; border: none; cursor: pointer;" href="{{ route("exact-important", $done_important -> id) }}"><i class="far fa-edit" style="color: #2a9d8f; font-size: 18px; margin: 0 10px 0 0;"></i></a>
                                    <form action="{{ route("delete-important", $done_important->id) }}" method="get">
                                        @csrf
                                        <button class="show_confirm" type="submit" style="background: none; border: none; cursor: pointer;"><i class="fas fa-trash-alt" style="color: tomato; font-size: 18px;"></i></button>
                                    </form>
                                </div>
        ...
        <div>
            <div class="todo_wrapper">
                <form action="{{ route("create-important") }}" method="post" class="todos">
                    @csrf
                    <div class="todo">
    ...
    <div class="edit_modal_wrapper">
        <div class="edit_modal">
            <form action="{{ route("update-important", $exact_important->id) }}" method="POST">
                <div class="description_block">
                    <input type="text" value="{{ $exact_important->description }}" name="description">
                </div>
                <div class="button_block">
                    @method("PUT")
                    @csrf
                    <input type="submit" value="Update">
                    <a href="/important">Close</a>
                </div>
            </form>
        </div>
        <div class="created_date">
            <p>Create on {{ $exact_important -> created_at }}</p>
            <form action="{{ route("delete-important", $exact_important->id) }}" method="GET">
